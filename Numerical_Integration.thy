...
definition int_approx_midpoint :: int_approx where
  \<open>int_approx_midpoint prec N f x a b vals = (let apprx = approx prec;
    ...
    in apprx (Var 0 *\<^sub>f sum_expr 1 N) (H # fs))\<close>

definition int_approx_simpson :: int_approx where
  \<open>int_approx_simpson prec N f x a b vals = (let apprx = approx prec;
...
definition
  \<open>int_approx_exp1_midpoint prec N \<equiv> (
    ...
    in approx prec (Var 0 +\<^sub>f Var 1) [err, approximation])\<close>

definition
  \<open>int_approx_exp1_simpson prec N \<equiv> (
...
definition extract_data :: "int_approx \<Rightarrow> floatarith \<Rightarrow> int \<Rightarrow> int \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> (nat \<times> nat \<times> float interval) list list" where
  \<open>extract_data ia f a b P I \<equiv> (let
    apprx = λp i. ia p i f 0 (ivl_coerce (of_int a)) (ivl_coerce (of_int b)) [ivl_coerce 0];
    precision_fns = map (λp i. (p, i, the (apprx p i))) (map (times 5) [1..<P]);
    int_count = map (λi. map (λf. f i) precision_fns) (map (times 5) [1..<I])
  in int_count)\<close>
...
definition extract_data_exp1 :: "(nat \<Rightarrow> nat \<Rightarrow> (float interval) option) \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> (nat \<times> nat \<times> float interval) list list" where
  \<open>extract_data_exp1 ia P I \<equiv> (let
    apprx = λp i. ia p i;
    precision_fns = map (λp i. (p, i, the (apprx p i))) (map (times 5) [1..<10]);
    res = map (λi. map (λf. f i) precision_fns) (map (times 5) [1..<10])
  in res)\<close>
